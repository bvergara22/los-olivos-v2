...
export function Header() {
  ...
  const serviciosColumns: DropdownColumn[] = [
    {
      ...
      items: [
        ...
        { label: "Parque cementerio", description: "Espacios de descanso eterno con naturaleza", href: "/parque-cementerio", icon: TreePine },
    ...
    },
    {
      ...
      accent: "var(--duelo-main)",
      items: [
        { label: "Pagos en línea", description: "Realiza tus pagos de forma segura y rápida", href: "https://pagos.losolivoscartagena.com/", isExternal: true, icon: CreditCard },
        { label: "Cotizar Homenaje", description: "Solicita una cotización personalizada", href: "/cotizar", icon: FileText, cotizar: true },
        { label: "Trámites fallecido", description: "Gestión de documentos y trámites necesarios", href: "/tramites", icon: ClipboardList, duelo: true },
      ]
    }
  ]
  ...
  const beneficiosColumns: DropdownColumn[] = [
    {
      ...
      items: [
        { label: "Planes Empresas", description: "Asistencia premium y paquete Solienvida para empresas", href: "/beneficios?tab=empresariales", icon: Building2 },
        { label: "Planes Personas", description: "Paquete Solientegral y Mascota Light para personas", href: "/beneficios?tab=personas", icon: Users },
      ]
    }
  ]
  ...
  const nosotrosColumns: DropdownColumn[] = [{ title: "Nosotros", items: conocenosItems }]
                      ...
                      className={`flex items-center gap-1 text-sm font-medium transition-colors py-2 ${
                        item.accent === "vida"
                          ...
                          : item.label === "Servicios"
                            ? openDropdown === item.label ? "text-duelo-main" : "text-muted-foreground hover:text-duelo-main"
            ...
            <div className="hidden lg:block absolute left-0 right-0 top-full bg-card border-b border-border shadow-lg">
              <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {conocenosItems.map((subItem, itemIndex) => (
                        <a key={itemIndex} href={subItem.href}
                ...
                {openDropdown === "Beneficios" && (
                  <div>
                    <h3 className="text-vida-dark font-semibold text-sm mb-4 uppercase tracking-wide">{beneficiosColumns[0].title}</h3>
                    <div className="grid grid-cols-2 gap-2">
                      {beneficiosColumns[0].items.map((subItem, itemIndex) => (
                        <a key={itemIndex} href={subItem.href} onClick={() => setOpenDropdown(null)}
                          className="flex items-start gap-3 p-3 rounded-lg hover:bg-muted transition-colors group"
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <span className="font-medium text-foreground text-sm group-hover:text-vida-dark transition-colors">{subItem.label}</span>
                            <p className="text-xs text-muted-foreground mt-0.5 line-clamp-2">{subItem.description}</p>
                          </div>
                        </a>
                      ))}
                ...
                {openDropdown === "Sedes & Planes" && !showSedesPanel && (
                  <div>
                    <h3 className="text-primary font-semibold text-sm mb-4 uppercase tracking-wide">{sedesColumns[0].title}</h3>
                    <div className="grid grid-cols-3 gap-2">
                      {sedesColumns[0].items.map((subItem, itemIndex) => (
                        subItem.href === "#ver-sedes" ? (
                          <button key={itemIndex} type="button" onClick={() => setShowSedesPanel(true)}
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <span className="font-medium text-foreground text-sm group-hover:text-primary transition-colors block">{subItem.label}</span>
                              <p className="text-xs text-muted-foreground mt-0.5 line-clamp-2">{subItem.description}</p>
                            </div>
                          </a>
                        )
                ...
                {openDropdown === "Sedes & Planes" && showSedesPanel && (
                  <div>
                    <button type="button" onClick={() => setShowSedesPanel(false)}
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                      {sedesItems.map((sede) => (
                        <Link key={sede.href} href={sede.href}
                          onClick={() => { setOpenDropdown(null); setShowSedesPanel(false) }}
                ...
                {openDropdown !== "Conócenos" && openDropdown !== "Beneficios" && openDropdown !== "Sedes & Planes" && navItems.find(item => item.label === openDropdown)?.columns && (
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
                    {navItems.find(item => item.label === openDropdown)?.columns?.map((column, colIndex) => (
                      <div key={colIndex}>
                        <h3 className="font-semibold text-sm mb-4 uppercase tracking-wide" style={{ color: column.accent ?? "var(--color-primary)" }}>
                          {column.title}
                        </h3>
                    ...
                    <>
                      <button
                        ...
                        className={`flex items-center justify-between w-full text-sm font-medium py-3 px-2 ${
                          item.accent === "vida"
                            ? openDropdown === item.label ? "text-vida-dark" : "text-foreground hover:text-vida-dark"
                            : item.label === "Servicios"
                              ? openDropdown === item.label ? "text-duelo-main" : "text-foreground hover:text-duelo-main"
                      ...
                      {openDropdown === item.label && (
                        <div className={`ml-2 border-l-2 pl-4 py-2 ${
                          item.accent === "vida" ? "border-vida-dark/20"
                            : item.label === "Servicios" ? "border-duelo-main/20"
                            : "border-primary/20"
                        }`}>
                          {item.label === "Beneficios" && (
                            <div className="space-y-1">
                              <p className="text-vida-dark font-semibold text-xs mb-2 uppercase tracking-wide">{beneficiosColumns[0].title}</p>
                              {beneficiosColumns[0].items.map((subItem, itemIndex) => (
                                <a key={itemIndex} href={subItem.href}
                          ...
                          {item.label !== "Beneficios" && item.label !== "Conócenos" && item.label !== "Sedes & Planes" && (
                            <div className="space-y-4">
                              {item.columns?.map((column, colIndex) => (
                                <div key={colIndex}>
                                  <p className="font-semibold text-xs mb-2 uppercase tracking-wide" style={{ color: column.accent ?? "var(--color-primary)" }}>
                                    {column.title}
                                  </p>
                                  <div className="space-y-1">
                                    {column.items.map((subItem, itemIndex) => (
                                      ...
                                      >
                                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${
                                          column.accent || subItem.duelo ? "bg-duelo-main/10 text-duelo-main"
                                            : subItem.cotizar ? "bg-cotizar-main/10 text-cotizar-main"
                                            : "bg-primary/10 text-primary"
                                        }`}>
                                          <subItem.icon className="w-4 h-4" />
                                        </div>
                                        <div className="flex items-center gap-1">
                                          <span className={column.accent || subItem.duelo ? "group-hover:text-duelo-main" : subItem.cotizar ? "group-hover:text-cotizar-main" : "group-hover:text-primary"}>
                                            {subItem.label}
                                          </span>
                                          {subItem.isExternal && <ExternalLink className="w-3 h-3" />}
                                        </div>
